...
def get_indices(
    ...
    limit = 2016 #2112
    print(os.environ.get("RESULT_DIR"))
    print(str(path).split("/")[-2])
    print(f"{str(path).split('/')[-1]}-metric.json")
...
def process_single(mode, indices, path, clean_path, attacked_path, quiet, limit):
    if mode.endswith("_fid"):
        ...
        clean_images = [
            Image.open(os.path.join(clean_path, f"{idx}.jpg")) for idx in indices
        ]
        attacked_images = [
            Image.open(os.path.join(attacked_path, f"{idx}.jpg")) for idx in indices
        ]
...
def main(path, dry, subset, quiet, limit=LIMIT, subset_limit=SUBSET_LIMIT):
    #path = "/home/ubuntu/verb-workspace/WAVES/datasets/attacked/imagenet/none-2-tree_ring"
    (
        dataset_name,
        attack_name,
        attack_strength,
...
if __name__ == "__main__":
    max_folders = 20  # Specify the maximum number of folders to generate
    paths = [f"/home/ubuntu/verb-workspace/WAVES/data/attacked/imagenet/{i}-2-real" for i in range(max_folders)]
    #/home/ubuntu/verb-workspace/WAVES/data/attacked/imagenet/rtest-2-real
    #"/home/ubuntu/verb-workspace/WAVES/data/attacked/imagenet/dfreefixed-2-real"]
    # "/home/ubuntu/verb-workspace/WAVES/data/attacked/imagenet/fixed-2-real"
    #"/home/ubuntu/verb-workspace/WAVES/datasets/attacked/imagenet/resnet18_pwm-2-tree_ring",
    #"/home/ubuntu/verb-workspace/WAVES/datasets/attacked/imagenet/resnet18_pwm-2-real"
    #"/home/ubuntu/verb-workspace/WAVES/datasets/attacked/imagenet/none-2-tree_ring", 
    #"/home/ubuntu/verb-workspace/WAVES/datasets/attacked/imagenet/none-2-real",           
    for p in paths:
        # Create a Click context to call the function directly
        with click.Context(main) as ctx:
            ctx.invoke(main, path=p)
